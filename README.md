...
- `ocr/` : 영수증 이미지를 업로드하면 OpenAI Vision API를 활용해 식자재 목록 추출
- `yolo/` : YOLO11 nano 모델로 이미지 속 식재료를 탐지 및 crop image 반환
...
├─ ocr/
│  └─ main_ocr.py              # OpenAI Vision API 기반 영수증 OCR 라우터
├─ yolo/
│  ├─ main_yolo.py             # YOLO11 감지 라우터
│  └─ weight/
│     └─ best.pt               # 학습된 YOLO 가중치
├─ llm/
│  ├─ main_llm.py              # 레시피 추천 전용 서버 
│  ├─ ingredient.json          # 냉장고 DB 재료 샘플 데이터
